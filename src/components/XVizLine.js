...
export class XVizLine extends BaseUIComponent {
	...
	updateChart(data) {
		...
		this.style.setProperty('--line-base-color', baseColor);
		...
		const svgWidth = 400;
		const svgHeight = 200;
		const leftPadding = 50; // Space for y-axis labels (increased)
		...
		for (let value = firstTick; value <= lastTick; value += step) {
			const y = svgHeight - bottomPadding - ((value - yAxisStart) / actualRange) * chartHeight;
		...
		const points = data.map((item, index) => {
			...
			const y = svgHeight - bottomPadding - ((item.value - yAxisStart) / actualRange) * chartHeight;
				...
				<svg viewBox="0 0 ${svgWidth} ${svgHeight}" class="viz-svg">
					...
					<defs>
						<pattern id="grid" width="40" height="20" patternUnits="userSpaceOnUse">
							<path d="M 40 0 L 0 0 0 20" fill="none" stroke="var(--paletteDivider, #e0e0e0)" stroke-width="0.5" opacity="0.3"/>
						</pattern>
					</defs>
